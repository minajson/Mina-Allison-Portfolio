        ...
        <div className="grid md:grid-cols-2 gap-8">
          {projects.map((p, i) => (
            <motion.article
              ...
              className="bg-slate-800/50 rounded-lg overflow-hidden border border-slate-700 hover:border-green-400 transition"
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="text-xl font-semibold text-white">{p.title}</h3>
                    <span className="mt-2 inline-block px-3 py-1 bg-green-400/15 text-green-400 text-xs rounded-full">
                      {p.category}
                    </span>
                  </div>
